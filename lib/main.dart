...
class _WaslaAppState extends State<WaslaApp> {
  ...
  @override Widget build(BuildContext context) => MaterialApp(debugShowCheckedModeBanner: false, title: 'وصلة | Wasla', themeMode: dark ? ThemeMode.dark : ThemeMode.light, theme: _theme(Brightness.light), darkTheme: _theme(Brightness.dark), home: HomePage(storage: storage, discovery: discovery, toggleTheme: () => setState(() => dark = !dark)));
...
class _HomePageState extends State<HomePage> {
  ...
  @override void initState() { super.initState(); widget.storage.deviceName().then((v) { if (mounted) setState(() => name = v); }); widget.discovery.devices.listen((d) { if (mounted) setState(() => devices = d); }); }
  ...
  Future<void> _pick({NearbyDevice? target}) async {
    ...
    final files = result.files.where((f) => f.path != null).map((f) => TransferFile(name: f.name, path: f.path!, size: f.size)).toList();
  ...
  @override Widget build(BuildContext context) => Directionality(textDirection: TextDirection.rtl, child: Scaffold(
    ...
    body: ListView(padding: const EdgeInsets.fromLTRB(20, 12, 20, 32), children: [
      ...
      const SizedBox(height: 7), Text('ملفاتك تنتقل من جهاز إلى جهاز عبر الشبكة المحلية.', style: TextStyle(color: Colors.grey)),
      const SizedBox(height: 28), Row(children: [Expanded(child: _Action(icon: Icons.upload_rounded, title: 'إرسال', subtitle: 'اختر ملفاتك', onTap: null)), const SizedBox(width: 12), Expanded(child: _Action(icon: Icons.download_rounded, title: 'استقبال', subtitle: 'انتظر اتصالاً', onTap: null))]),
      const SizedBox(height: 10), Row(children: [Expanded(child: FilledButton.icon(onPressed: () => _pick(), icon: const Icon(Icons.upload_rounded), label: const Text('إرسال ملفات'))), const SizedBox(width: 10), Expanded(child: OutlinedButton.icon(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => ReceivePage(storage: widget.storage, discovery: widget.discovery)), icon: const Icon(Icons.download_rounded), label: const Text('وضع الاستقبال')))]),
      const SizedBox(height: 30), Row(children: [const Icon(Icons.radar_rounded), const SizedBox(width: 8), const Text('أجهزة قريبة', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800)), const Spacer(), Text('${devices.length}')]),
      const SizedBox(height: 10), if (devices.isEmpty) const _EmptyNearby() else ...devices.map((d) => _DeviceTile(device: d, onTap: () => _pick(target: d))),
      const SizedBox(height: 20), const _PrivacyStrip(),
    ]),
  ));
}
...
class _SendPageState extends State<SendPage> {
  ...
  Future<void> _start() async {
    ...
    if (socket == null) { setState(() { running = false; status = 'تعذر الاتصال بالجهاز'; }); return; }
    try {
