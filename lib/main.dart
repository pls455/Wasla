...
class WaslaApp extends StatefulWidget { const WaslaApp({super.key}); @override State<WaslaApp> createState() => _WaslaAppState(); }
class _WaslaAppState extends State<WaslaApp> {
  final storage = StorageService(); late final discovery = DiscoveryService(storage); bool dark = true;
...
class _HomePageState extends State<HomePage> {
  List<NearbyDevice> devices = []; String name = 'هاتف وصلة';
...
class _ReceivePageState extends State<ReceivePage> {
  ...
  Future<void> _accept() async {
    final socket = pending; final input = reader; final o = offer; if (socket == null || input == null || o == null) return; setState(() => busy = true);
    final files = (o['files'] as List).map((e) => TransferFile(name: e['name'], path: '', size: (e['size'] as num).toInt(), hash: e['hash'])).toList();
    try { await TransferService(widget.storage).receive(socket: socket, reader: input, files: files, onProgress: (p) { if (mounted) setState(() => progress = p); }); await widget.storage.addHistory(TransferRecord(id: DateTime.now().toIso8601String(), direction: TransferDirection.received, device: o['sender'] ?? 'جهاز', totalBytes: files.fold(0, (a, b) => a + b.size), completedBytes: files.fold(0, (a, b) => a + b.size), status: 'success', createdAt: DateTime.now())); }
    catch (e) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(e.toString()))); }
    finally { await socket.close(); if (mounted) setState(() { busy = false; pending = null; reader = null; offer = null; }); }
  }
  @override Widget build(BuildContext context) => Directionality(textDirection: TextDirection.rtl, child: Scaffold(appBar: AppBar(title: const Text('استقبال')), body: ListView(padding: const EdgeInsets.all(20), children: [if (offer != null) _IncomingCard(offer: offer!, onAccept: _accept, onReject: () async { await pending?.close(); if (mounted) setState(() { pending = null; reader = null; offer = null; }); }), if (offer == null && progress == null) const _EmptyReceive(), if (progress != null) _TransferCard(progress: progress, files: const [], status: progress!.status)])));
}
