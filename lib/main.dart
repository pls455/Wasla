...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _pickFiles({NearbyDevice? target}) async {
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => SendPage(storage: widget.storage, discovery: widget.discovery, files: files, target: target),
      ),
    );
  }
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(target == null ? 'اختر المحتوى ثم الجهاز.' : 'إلى ${target.name}'),
                const SizedBox(height: 16),
                _ActionTile(icon: Icons.folder_rounded, title: 'ملفات', subtitle: 'صور، فيديو، مستندات وغيرها', onTap: () { Navigator.pop(sheet); _pickFiles(target: target); }),
                _ActionTile(icon: Icons.apps_rounded, title: 'تطبيقات', subtitle: 'اختر تطبيقات مثبتة على الهاتف', onTap: () { Navigator.pop(sheet); _openApps(target: target); }),
              ],
            ),
          ),
        ),
        ...
        child: Scaffold(
          appBar: AppBar(
            title: const Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('وصلة', style: TextStyle(fontSize: 25, fontWeight: FontWeight.w900)),
              Text('WASLA', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, letterSpacing: 2)),
            ]),
            actions: [
              IconButton(onPressed: widget.onToggleTheme, tooltip: 'المظهر', icon: const Icon(Icons.contrast_rounded)),
              IconButton(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => SettingsPage(storage: widget.storage))), tooltip: 'الإعدادات', icon: const Icon(Icons.settings_outlined)),
            ],
          ),
          body: RefreshIndicator(
            onRefresh: widget.discovery.start,
            child: ListView(
              ...
              children: [
                ...
                const Text('نقل محلي مباشر، بدون خادم وسيط.', style: TextStyle(fontSize: 14)),
                const SizedBox(height: 24),
                Row(children: [
                  Expanded(child: _BigAction(icon: Icons.arrow_upward_rounded, title: 'إرسال', subtitle: 'ملفات أو تطبيقات', primary: true, onTap: _sendOptions)),
                  const SizedBox(width: 12),
                  Expanded(child: _BigAction(icon: Icons.arrow_downward_rounded, title: 'استقبال', subtitle: 'انتظر طلبًا', onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => ReceivePage(storage: widget.storage, discovery: widget.discovery))))),
                ]),
                const SizedBox(height: 28),
                Row(children: [const Icon(Icons.radar_rounded, size: 21), const SizedBox(width: 8), const Text('الأجهزة القريبة', style: TextStyle(fontSize: 19, fontWeight: FontWeight.w800)), const Spacer(), Text('${devices.length}')]),
                const SizedBox(height: 10),
                if (devices.isEmpty)
                  const _EmptyNearby()
                else
                  ...devices.map((device) => Padding(padding: const EdgeInsets.only(bottom: 8), child: _DeviceTile(device: device, onTap: () => _sendOptions(target: device)))),
                const SizedBox(height: 14),
                Container(
                  padding: const EdgeInsets.all(16),
                  decoration: BoxDecoration(color: Theme.of(context).colorScheme.surfaceContainerHighest.withValues(alpha: .55), borderRadius: BorderRadius.circular(22)),
                  child: Row(children: [const Icon(Icons.lock_outline_rounded), const SizedBox(width: 12), Expanded(child: Text('هذا الجهاز: $deviceName\nالنقل محلي ومباشر.', style: const TextStyle(fontWeight: FontWeight.w700)))]),
                ),
              ],
            ),
          ),
...
class _SendPageState extends State<SendPage> {
  final transfer = TransferService(StorageService());
  NearbyDevice? selected;
  TransferProgress? progress;
  String status = 'اختر جهازًا للبدء';
  bool running = false;
  List<NearbyDevice> devices = const [];
  ...
  Future<void> _start() async {
    ...
    if (socket == null) {
      if (mounted) setState(() { running = false; status = 'تعذر الاتصال بالجهاز'; });
      return;
    }
    try {
      ...
      await widget.storage.addHistory(TransferRecord(id: '${DateTime.now().microsecondsSinceEpoch}', direction: TransferDirection.sent, device: device.name, totalBytes: widget.files.fold(0, (a, b) => a + b.size), completedBytes: widget.files.fold(0, (a, b) => a + b.size), status: 'completed', createdAt: DateTime.now()));
    } catch (e) {
      if (mounted) setState(() => status = _error(e));
    } finally {
      socket.destroy();
  ...
  Widget build(BuildContext context) => Directionality(
        ...
        child: Scaffold(
          ...
          body: ListView(padding: const EdgeInsets.all(20), children: [
            _TransferSummary(files: widget.files),
            const SizedBox(height: 18),
            if (selected == null) ...[
              const Text('اختر الجهاز', style: TextStyle(fontSize: 19, fontWeight: FontWeight.w800)),
              const SizedBox(height: 10),
              if (devices.isEmpty) const _EmptyNearby() else ...devices.map((d) => Padding(padding: const EdgeInsets.only(bottom: 8), child: _DeviceTile(device: d, selected: selected?.id == d.id, onTap: () => setState(() => selected = d)))),
            ] else
              _DeviceTile(device: selected!, selected: true, onTap: running ? null : () => setState(() => selected = null)),
            const SizedBox(height: 18),
            if (progress != null) _ProgressCard(progress: progress!),
            const SizedBox(height: 12),
            Text(status, textAlign: TextAlign.center),
            const SizedBox(height: 18),
            FilledButton.icon(onPressed: selected == null || running ? null : _start, icon: Icon(running ? Icons.sync_rounded : Icons.send_rounded), label: Text(running ? 'جاري الإرسال...' : 'ابدأ الإرسال')),
          ]),
        ),
      );
}
...
class _ReceivePageState extends State<ReceivePage> {
  ...
  Future<void> _listen() async {
    ...
    _sub = server!.listen(_handleSocket);
  }
  ...
  Future<void> _handleSocket(Socket socket) async {
    ...
    try {
      ...
      await widget.storage.addHistory(TransferRecord(id: '${DateTime.now().microsecondsSinceEpoch}', direction: TransferDirection.received, device: offer['sender']?.toString() ?? 'جهاز قريب', totalBytes: files.fold(0, (a, b) => a + b.size), completedBytes: files.fold(0, (a, b) => a + b.size), status: 'completed', createdAt: DateTime.now()));
    } catch (e) {
      if (mounted) setState(() => status = _error(e));
    } finally {
      socket.destroy();
  ...
  @override
  Widget build(BuildContext context) => Directionality(textDirection: TextDirection.rtl, child: Scaffold(appBar: AppBar(title: const Text('استقبال', style: TextStyle(fontWeight: FontWeight.w900))), body: Center(child: Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(progress == null ? Icons.wifi_tethering_rounded : Icons.download_done_rounded, size: 70), const SizedBox(height: 20), Text(status, textAlign: TextAlign.center, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w800)), if (progress != null) ...[const SizedBox(height: 20), _ProgressCard(progress: progress!)] else ...[const SizedBox(height: 12), const Text('افتح وصلة على الجهاز الآخر واختر هذا الجهاز.')]]))));
}
...
class _AppPickerPageState extends State<AppPickerPage> {
  ...
  Future<void> _load() async {
    try {
      final value = await loadInstalledApps();
  ...
  Future<void> _prepare() async {
    ...
    for (final app in apps.where((a) => selected.contains(a.packageName))) {
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality(textDirection: TextDirection.rtl, child: Scaffold(
      appBar: AppBar(title: const Text('التطبيقات', style: TextStyle(fontWeight: FontWeight.w900)), actions: [if (selected.isNotEmpty) Padding(padding: const EdgeInsets.all(14), child: Center(child: Text('${selected.length} محدد')))]),
      ...
      body: loading ? const Center(child: CircularProgressIndicator()) : ListView(padding: const EdgeInsets.all(20), children: [TextField(onChanged: (v) => setState(() => query = v), decoration: const InputDecoration(hintText: 'ابحث عن تطبيق', prefixIcon: Icon(Icons.search_rounded))), const SizedBox(height: 14), Text('${shown.length} تطبيق متاح'), const SizedBox(height: 8), ...shown.map((app) => Padding(padding: const EdgeInsets.only(bottom: 8), child: _AppTile(app: app, selected: selected.contains(app.packageName), onTap: () => setState(() => selected.contains(app.packageName) ? selected.remove(app.packageName) : selected.add(app.packageName)))), if (shown.isEmpty) const Padding(padding: EdgeInsets.only(top: 70), child: Center(child: Text('لم نجد تطبيقًا بهذا الاسم')))]),
    ));
  }
}
...
class _SettingsPageState extends State<SettingsPage> {
  ...
  Future<void> _save() async {
    ...
    await widget.storage.setDeviceName(controller.text);
    if (mounted) { setState(() => saving = false); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم حفظ اسم الجهاز.'))); }
  }
...
class _BigAction extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Card(color: primary ? Theme.of(context).colorScheme.primaryContainer : Theme.of(context).colorScheme.surfaceContainerHighest, child: InkWell(borderRadius: BorderRadius.circular(16), onTap: onTap, child: Padding(padding: const EdgeInsets.all(18), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Icon(icon, size: 30), const SizedBox(height: 20), Text(title, style: const TextStyle(fontSize: 21, fontWeight: FontWeight.w900)), const SizedBox(height: 4), Text(subtitle, style: const TextStyle(fontSize: 12))]))));
}
...
class _ProgressCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) { final value = progress.total == 0 ? 0.0 : (progress.bytes / progress.total).clamp(0.0, 1.0); return Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(children: [LinearProgressIndicator(value: value, minHeight: 8, borderRadius: BorderRadius.circular(8)), const SizedBox(height: 10), Row(children: [Text('${(value * 100).round()}%'), const Spacer(), Text('${_fmt(progress.speed)}/ث')]), if (progress.eta != null) Text('متبقي تقريبًا ${_duration(progress.eta!)}')]))); }
}

class _AppTile extends StatelessWidget {
  const _AppTile({required this.app, required this.selected, required this.onTap});
  final AppItem app; final bool selected; final VoidCallback onTap;
  @override
  Widget build(BuildContext context) => Card(child: ListTile(onTap: onTap, leading: CircleAvatar(child: Icon(Icons.apps_rounded)), title: Text(app.name, style: const TextStyle(fontWeight: FontWeight.w800)), subtitle: Text('${app.packageName}\n${_fmt(app.size)}'), isThreeLine: true, trailing: Icon(selected ? Icons.check_circle_rounded : Icons.radio_button_unchecked_rounded)));
}
...
String _duration(Duration d) { final s = d.inSeconds; if (s < 60) return '${s}ث'; if (s < 3600) return '${s ~/ 60}د'; return '${s ~/ 3600}س'; }
String _error(Object e) => e is StateError ? e.message ?? 'حدث خطأ' : 'حدث خطأ أثناء النقل.';
