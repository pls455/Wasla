...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _pickFiles({NearbyDevice? target}) async {
    ...
    final files = picked.where((f) => f.path != null).map((f) {
      final path = f.path!;
      return TransferFile(name: f.name, path: path, size: File(path).lengthSync());
    }).toList();
  ...
  void _openApps({NearbyDevice? target}) {
    Navigator.push(context, MaterialPageRoute(builder: (_) => AppPickerPage(onReady: (files) {
      Navigator.push(context, MaterialPageRoute(builder: (_) => SendPage(storage: widget.storage, discovery: widget.discovery, files: files, target: target)));
    })));
  }
        ...
        body: RefreshIndicator(
          ...
          child: ListView(
            ...
            children: [
              ...
              if (devices.isEmpty) const _EmptyNearby() else ...devices.map((device) => Padding(padding: const EdgeInsets.only(bottom: 8), child: _DeviceTile(device: device, onTap: () => _sendOptions(target: device)))),
...
class _ReceivePageState extends State<ReceivePage> {
  ...
  Future<void> _handleSocket(Socket socket) async {
    ...
    try {
      ...
      final files = raw.map((e) {
        final m = Map<String, dynamic>.from(e as Map);
        return TransferFile(name: m['name'] as String, path: '', size: (m['size'] as num).toInt(), hash: m['hash'] as String?);
      }).toList();
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(status, textAlign: TextAlign.center, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w800)),
                if (progress != null) ...[
                  const SizedBox(height: 20),
                  _ProgressCard(progress: progress!),
                ] else ...[
                  const SizedBox(height: 12),
                  const Text('افتح وصلة على الجهاز الآخر واختر هذا الجهاز.', textAlign: TextAlign.center),
                ],
              ],
            ),
          ),
...
class _SettingsPageState extends State<SettingsPage> {
  ...
  Future<void> _save() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم حفظ اسم الجهاز.')));
  }
