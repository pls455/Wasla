...
class HomePage extends StatefulWidget {
  ...
  final StorageService storage; final DiscoveryService discovery; final VoidCallback onToggleTheme;
...
class _SheetAction extends StatelessWidget{const _SheetAction({required this.icon,required this.title,required this.subtitle,required this.onTap});final IconData icon;final String title,subtitle;final VoidCallback onTap;@override Widget build(BuildContext c)=>ListTile(onTap:onTap,contentPadding:const EdgeInsets.symmetric(horizontal:4,vertical:4),leading:Container(width:52,height:52,decoration:BoxDecoration(color:Theme.of(c).colorScheme.primaryContainer,borderRadius:BorderRadius.circular(17)),child:Icon(icon)),title:Text(title,style:const TextStyle(fontWeight:FontWeight.w800,fontSize:16)),subtitle:Text(subtitle));}
class _DeviceTile extends StatelessWidget{const _DeviceTile({required this.device,required this.onTap});final NearbyDevice device;final VoidCallback onTap;@override Widget build(BuildContext c)=>Material(color:Theme.of(c).colorScheme.surfaceContainerHighest,borderRadius:BorderRadius.circular(20),child:InkWell(onTap:onTap,borderRadius:BorderRadius.circular(20),child:Padding(padding:const EdgeInsets.all(12),child:Row(children:[const CircleAvatar(radius:24,child:Icon(Icons.devices_rounded)),const SizedBox(width:12),Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(device.name,style:const TextStyle(fontWeight:FontWeight.w800)),const SizedBox(height:3),Text('جاهز للاتصال',style:TextStyle(fontSize:12,color:Theme.of(c).colorScheme.onSurfaceVariant))])),const Icon(Icons.chevron_left_rounded)]))));}
class _EmptyNearby extends StatelessWidget{const _EmptyNearby();@override Widget build(BuildContext c)=>Container(padding:const EdgeInsets.all(22),decoration:BoxDecoration(borderRadius:BorderRadius.circular(22),border:Border.all(color:Theme.of(c).dividerColor)),child:const Column(children:[Icon(Icons.wifi_find_rounded,size:38),SizedBox(height:8),Text('لا توجد أجهزة قريبة',style:TextStyle(fontWeight:FontWeight.w800)),SizedBox(height:5),Text('افتح وصلة على الجهاز الآخر وتأكد من الاتصال بالشبكة نفسها.',textAlign:TextAlign.center,style:TextStyle(fontSize:13))]));}
class _PrivacyStrip extends StatelessWidget{const _PrivacyStrip({required this.deviceName});final String deviceName;@override Widget build(BuildContext c)=>Container(padding:const EdgeInsets.all(15),decoration:BoxDecoration(borderRadius:BorderRadius.circular(20),color:Theme.of(c).colorScheme.surfaceContainerHighest),child:Row(children:[const Icon(Icons.lock_outline_rounded,size:20),const SizedBox(width:10),Expanded(child:Text('هذا الجهاز: $deviceName\nالنقل محلي ومباشر.',style:const TextStyle(fontSize:12,height:1.5)))]));}
class _TransferCard extends StatelessWidget{const _TransferCard({required this.progress,required this.files,required this.status});final TransferProgress? progress;final List<TransferFile> files;final String status;@override Widget build(BuildContext c){final p=progress;final ratio=p==null||p.total==0?0.0:p.bytes/p.total;return Container(padding:const EdgeInsets.all(22),decoration:BoxDecoration(borderRadius:BorderRadius.circular(28),color:Theme.of(c).colorScheme.surfaceContainerHighest),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Icon(p?.status.contains('اكتمل')==true?Icons.check_circle_rounded:Icons.bolt_rounded,size:38),const SizedBox(height:14),Text(status,style:const TextStyle(fontSize:21,fontWeight:FontWeight.w900)),if(files.isNotEmpty)Padding(padding:const EdgeInsets.only(top:5),child:Text('${files.length} ملف • ${_fmt(files.fold(0,(a,b)=>a+b.size))}')),const SizedBox(height:18),LinearProgressIndicator(value:p==null?null:ratio,minHeight:8,borderRadius:BorderRadius.circular(8)),if(p!=null)Padding(padding:const EdgeInsets.only(top:10),child:Text('${_fmt(p.bytes)} / ${_fmt(p.total)}  •  ${_fmt(p.speed.round())}/ث${p.eta==null?'':'  •  ${p.eta!.inSeconds} ث تقريباً'}',style:TextStyle(color:Theme.of(c).colorScheme.onSurfaceVariant,fontSize:13))) ]));}}
class _IncomingCard extends StatelessWidget{const _IncomingCard({required this.offer,required this.onAccept,required this.onReject});final Map<String,dynamic> offer;final VoidCallback onAccept,onReject;@override Widget build(BuildContext c){final files=offer['files'] as List;return Container(padding:const EdgeInsets.all(20),decoration:BoxDecoration(borderRadius:BorderRadius.circular(28),color:Theme.of(c).colorScheme.surfaceContainerHighest),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[const Icon(Icons.file_download_outlined,size:36),const SizedBox(height:12),const Text('طلب استقبال',style:TextStyle(fontSize:22,fontWeight:FontWeight.w900)),const SizedBox(height:5),Text('${offer['sender']??'جهاز قريب'} يريد إرسال ${files.length} ملف'),const SizedBox(height:12),...files.map((f)=>ListTile(contentPadding:EdgeInsets.zero,leading:const Icon(Icons.insert_drive_file_outlined),title:Text(f['name']),subtitle:Text(_fmt((f['size'] as num).toInt())))),const SizedBox(height:8),Row(children:[Expanded(child:OutlinedButton(onPressed:onReject,child:const Text('رفض'))),const SizedBox(width:10),Expanded(child:FilledButton(onPressed:onAccept,child:const Text('استقبال')))])]));}}
class _EmptyReceive extends StatelessWidget{const _EmptyReceive();@override Widget build(BuildContext c)=>const Padding(padding:EdgeInsets.only(top:80),child:Column(children:[Icon(Icons.inbox_rounded,size:65),SizedBox(height:15),Text('جاهز للاستقبال',style:TextStyle(fontSize:23,fontWeight:FontWeight.w900)),SizedBox(height:6),Text('عندما يرسل إليك جهاز قريب سيظهر الطلب هنا قبل بدء النقل.',textAlign:TextAlign.center)]));}
class _AppTile extends StatelessWidget{const _AppTile({required this.app,required this.selected,required this.onTap});final AppItem app;final bool selected;final VoidCallback onTap;@override Widget build(BuildContext c){ImageProvider? image;try{if(app.iconBase64!=null)image=MemoryImage(base64Decode(app.iconBase64!));}catch(_){ }return Material(color:selected?Theme.of(c).colorScheme.primaryContainer:Theme.of(c).colorScheme.surfaceContainerHighest,borderRadius:BorderRadius.circular(19),child:InkWell(onTap:onTap,borderRadius:BorderRadius.circular(19),child:Padding(padding:const EdgeInsets.all(10),child:Row(children:[Container(width:48,height:48,decoration:BoxDecoration(borderRadius:BorderRadius.circular(14),image:image==null?null:DecorationImage(image:image,fit:BoxFit.cover),color:Theme.of(c).colorScheme.surface),child:image==null?const Icon(Icons.apps_rounded):null),const SizedBox(width:12),Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(app.name,maxLines:1,overflow:TextOverflow.ellipsis,style:const TextStyle(fontWeight:FontWeight.w800)),Text(_fmt(app.size),style:TextStyle(fontSize:12,color:Theme.of(c).colorScheme.onSurfaceVariant))])),Checkbox(value:selected,onChanged:(_)=>onTap())]))));}}
String _fmt(int bytes){const units=['B','KB','MB','GB','TB'];var n=bytes.toDouble();var i=0;while(n>=1024&&i<units.length-1){n/=1024;i++;}return '${n<10&&i>0?n.toStringAsFixed(1):n.round()} ${units[i]}';}
String _humanError(Object e){final s=e.toString().replaceFirst('Bad state: ','');if(s.contains('لم يتم قبول'))return 'تم رفض عملية النقل.';if(s.contains('انقطع'))return 'انقطع الاتصال. يمكنك المحاولة مجددًا.';if(s.contains('التحقق'))return 'تعذر التحقق من الملف بعد النقل.';return s;}
